...
const Dashboard = () => {
  ...
  const percentToday = today?.total ? Math.round((today.attended / today.total) * 100) : 0;
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {courses.length === 0 ? (
            ...
            courses.map((course) => (
              ...
              >
                <div className={`h-48 rounded-xl bg-gradient-to-r ${getRandomGradient()}`} />
            ...
            {today?.classes?.map((cls, i) => (
              ...
              >
                <span className="flex items-center gap-2">
                  <span
                    className={`w-3 h-3 rounded-full ${cls.attended ? "bg-purple-300" : "bg-purple-100"}`}
                  />
                  {cls.name}
                </span>
